...
func searchMimeTypeByExt(ext string, defaultMimeType string, atvext ...string) (mimeType string, activeExt bool) {
	...
	if _, foundok := mimeTypesFound[ext]; foundok {
		mimeType = mimeTypesFound[ext]
	} else {
		var readerhndl = RegisteredEmbededReader("/mimetypes.tsv")
		if readerhndl == nil {
			regsieterEmbeddedMimeReader()
			readerhndl = RegisteredEmbededReader("/mimetypes.tsv")
		}
		if readerhndl != nil {
			var mimeReader, _ = goio.NewIORW(readerhndl())
			for {
				if mimeLine, _ := mimeReader.Readln(); mimeLine != "" {
					var mime = strings.Split(mimeLine, "\t")
					if mime[2] == ext {
						mimeTypesFound[ext] = mime[1]
						mimeType = mimeTypesFound[ext]
						break
					}
				} else {
		...
		if _, foundok := mimeTypesFound[ext]; !foundok {
			mimeTypesFound[ext] = defaultMimeType
		}
	}
	if _, foundActiveExt := activeExtFound[ext]; foundActiveExt {
		activeExt = activeExtFound[ext]
	} else {
		for _, atvxt := range atvext {
			if atvxt == ext {
				activeExtFound[ext] = true
				activeExt = true
				return
			}
		}
		if _, foundActiveExt := activeExtFound[ext]; !foundActiveExt {
			activeExtFound[ext] = false
			activeExt = false
		}
	}
	return
...
var activeExtFound = map[string]bool{}
var mimeTypesFound = map[string]string{}
var mimeTypesLock = sync.RWMutex{}

func regsieterEmbeddedMimeReader() {
	mimeTypesLock.RLock()
	defer mimeTypesLock.RUnlock()
	RegisterEmbededReaders(
		"/mimetypes.tsv", MimetypesTSV)
}

const mimetypestsv string = `3D Crossword Plugin	application/vnd.hzn-3d-crossword	.x3d	IANA: 3D Crossword Plugin
...
func MimetypesTSV() io.Reader {
	return strings.NewReader(mimetypestsv)
}
